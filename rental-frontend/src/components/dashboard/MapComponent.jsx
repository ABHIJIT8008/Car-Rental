...
const MapComponent = ({ pickup, dropoff }) => {
  ...
  useEffect(() => {
    ...
    pickupMarkerRef.current = L.marker([pickLat, pickLon])
      .addTo(map)
      .bindPopup('📍 Pickup Location');
    dropoffMarkerRef.current = L.marker([dropLat, dropLon])
      .addTo(map)
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full rounded-2xl border border-gray-700"
      style={{ minHeight: 300 }}
    />
  );
};
