...
import {
  ...
  DriverRideCancelledPanel,
} from "./DriverPanels.jsx";
...
const PanelRenderer = (props) => {
  ...
  if (user.role === "driver") {
    ...
    if (!driverProfile?.vehicleDetails?.model) {
      return (
        <DriverOnboardingPanel
          api={props.api}
          setDriverProfile={props.setDriverProfile}
        />
      );
    }
    if (activeDriverRide) {
      // If the assigned ride has been cancelled by the user, show a specific panel
      if (activeDriverRide.status === "cancelled") {
        return (
          <DriverRideCancelledPanel
            onFindRides={() => {
              // clear the active ride and refresh pending rides so driver returns to dashboard
              if (props.setActiveDriverRide) props.setActiveDriverRide(null);
              if (props.fetchPendingRides) props.fetchPendingRides();
            }}
          />
        );
      }
          ...
          <div className="bg-green-900/50 p-6 rounded-xl space-y-4 flex-grow">
            ...
            <div className="flex justify-between items-center text-lg">
              ...
              <p className="font-bold text-white">
                ${activeDriverRide.estimatedFare}
              </p>
            </div>
          </div>
          <button
            onClick={() => props.setActiveDriverRide(null)}
            className="w-full h-14 mt-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 px-4 rounded-lg transition"
          >
            Complete Ride (dev)
          </button>
        </div>
    ...
    return (
      <DriverDashboardPanel
        ...
        isDriverDataLoading={isDriverDataLoading}
  ...
  switch (rideStatus) {
    case "searching":
      return <UserSearchingPanel resetRide={resetRide} />;
    case "confirmed":
      return (
        <UserConfirmedPanel
          resetRide={resetRide}
